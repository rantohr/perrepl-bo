...
const OrderForm: FC<{ onConfirm: (data: IOrder) => void, onCancel: () => void }> = ({ onConfirm, onCancel }) => {
  ...
  useEffect(() => {
    if (selectedItem) {
      const cleanData = {
        ...selectedItem,
        travelers: selectedItem.travelers?.length ? selectedItem.travelers : [({} as any)],
      };

      setFormData({
        initialData: cleanData,
        fields: [
          { field: 'travelers_0_first_name', label: "Prénom(s)", required: true, size: 1 },
          { field: 'travelers_0_last_name', label: "Nom", required: true, size: 2 },
          { field: 'travelers_0_email', label: "Email", required: true, size: 2 },
          { field: 'travelers_0_phone_number', label: "Téléphone", required: true, size: 1 },
          { field: 'arrival_datetime', label: "Date d'arrivée", required: true, size: 1, type: 'date', onChange: onChangeArrival },
          { field: 'departure_datetime', label: "Date de départ", required: true, size: 1, type: 'date', onChange: onChangeDeparture },
          { field: 'trip_duration', label: "Nombre de jour", required: true, size: 1, type: 'number', onChange: onChangeDuration },
  ...
  const beforeConfirm = (data: any) => {
    ...
    if (onConfirm !== undefined) onConfirm(data_c);
  };
