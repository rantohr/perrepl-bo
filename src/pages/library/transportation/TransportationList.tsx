...
import { getTransportations } from "../../../services/transportation.service";
...
const TransportationList: FC = () => {
    ...
    const [rows, setRows] = useState<ITransportation[]>([]);
    useEffect(() => {
      loadData();
    }, []);
        ...
        <div className="flex flex-col gap-3">
            {rows.map((filter, filter_index) => (
                <GenericCard
                    ...
                    place= {filter.locations && filter.locations[0].province}
                    mark={marks.activity}
                />
            ))}
        </div>
